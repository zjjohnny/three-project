            ...
            <table class="goods_data_box_header table2" v-for="(item) in filterList" :key="item.id">
                <tr class="goods_item_head">
                    <td class="goods_title" colspan="7">
                        ...
                        <span>订单编号：{{ item.o_oderNumber }}</span>
                        <span>下单时间：{{ item.o_orderTime }}</span>
                        <span>买家：{{ item.o_buyer }}</span>
                    </td>
                </tr>
                <tr class="goods_item_con">
                    <td width="30%">
                        <div id="goods_container_img">
                            ...
                            <div class="goods_container_title">
                                <span>{{ item.o_productName }}【交易快照】</span>
                                <span>交易保障</span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td width="8%">{{ item.o_quantity }}</td> <!-- 商品数量 -->
                    <td width="9%"></td> <!-- 售后 -->
                    <td width="17%" :class="item.o_state == '待付款' ? 'no_pay' : ''">{{ item.o_state }}</td>
                    <td width="16%">{{ item.o_price }}*{{ item.o_quantity }}</td> <!-- 商品总价 -->
                    <td width="12%" class="goods_caozuo">
                        <button v-if="item.o_state == '待付款'">修改价格</button>
                        <button v-else-if="item.o_state == '待发货'">发货</button>
                        <button
                            v-else-if="item.o_state == '待收货' || item.o_state == '待评价' || item.o_state == '已完成'">查看物流</button>
                        <button
                            v-if="item.o_state == '待付款' || item.o_state == '待发货' || item.o_state == '待收货' || item.o_state == '待评价'">取消订单</button>
                    </td>
                </tr>
                <tr class="zhanwei1">
                    <td colspan="7"></td>
...
export default {
    ...
    computed: {
        ...
        filterList(){
            if(this.state == '待发货' || this.state == '待收货' || this.state == '待评价' || this.state == '已完成' || this.state == '已取消'){
                return this.goodsList.filter((item)=>{
                    return item.o_state == this.state
                })
            }else{
